...
function hasMatchingUserMessage(target: UiMessage, messages: UiMessage[]): boolean {
  if (target.role !== 'user') return false
  const targetText = normalizeMessageText(target.text)
  const targetImages = Array.isArray(target.images) ? target.images : []
  const targetFileCount = Array.isArray(target.fileAttachments) ? target.fileAttachments.length : 0
  ...
  return messages.some((message) => {
    if (message === target || message.role !== 'user') return false
    const messageText = normalizeMessageText(message.text)
    const messageImages = Array.isArray(message.images) ? message.images : []
    const messageFileCount = Array.isArray(message.fileAttachments) ? message.fileAttachments.length : 0
    const messageSkillCount = Array.isArray(message.skills) ? message.skills.length : 0
...
export function useDesktopState() {
  ...
  type AcknowledgedDesktopTurnState = {
    turnId: string
  }
  ...
  const acknowledgedDesktopTurnByThreadId = ref<Record<string, AcknowledgedDesktopTurnState>>({})
  ...
  function pruneThreadScopedState(flatThreads: UiThread[]): void {
    ...
    acknowledgedDesktopTurnByThreadId.value = pruneThreadStateMap(
      acknowledgedDesktopTurnByThreadId.value,
      activeThreadIds,
    )
  ...
  function acknowledgeDesktopTurn(threadId: string, turnId: string): void {
  ...
  function clearAcknowledgedDesktopTurn(threadId: string): void {
    if (!threadId || !acknowledgedDesktopTurnByThreadId.value[threadId]) return
    acknowledgedDesktopTurnByThreadId.value = omitKey(acknowledgedDesktopTurnByThreadId.value, threadId)
  }

  function shouldPreserveAcknowledgedDesktopTurn(
    threadId: string,
    loadedInProgress: boolean,
    force: boolean,
  ): boolean {
    if (loadedInProgress || force) return false
    const desktopTurn = acknowledgedDesktopTurnByThreadId.value[threadId]
    if (!desktopTurn) return false
    return activeTurnIdByThreadId.value[threadId] === desktopTurn.turnId
  }
  ...
  function readBridgeStringArray(value: unknown): string[] {
  ...
  function readBridgeFileAttachments(value: unknown): FileAttachment[] {
    if (!Array.isArray(value)) return []
    const attachments: FileAttachment[] = []
    for (const item of value) {
      const record = asRecord(item)
      const fsPath = readString(record?.fsPath).trim()
      const path = readString(record?.path).trim() || fsPath
      const label = readString(record?.label).trim() || path.split(/[\\/]+/u).pop() || fsPath
  ...
  function readBridgeSubmittedUserMessage(notification: RpcNotification): { threadId: string; message: UiMessage } | null {
    if (notification.method !== 'bridge/user-message-submitted') return null
    ...
    const submissionId = readString(params.submissionId).trim() || `${Date.now()}`
    const text = readString(params.text).trim()
    const imageUrls = readBridgeStringArray(params.imageUrls)
    const skills = readBridgeSkills(params.skills)
    const fileAttachments = readBridgeFileAttachments(params.fileAttachments)
    ...
    return {
      threadId,
      message: {
        id: `bridge-user:${threadId}:${submissionId}`,
        role: 'user',
        text,
        images: imageUrls.length > 0 ? imageUrls : undefined,
  ...
  function updateThreadTimestamp(
    groups: UiProjectGroup[],
    threadId: string,
    updatedAtIso: string,
  ): UiProjectGroup[] {
    ...
    const nextGroups = groups.map((group) => {
      ...
      const nextThreads = group.threads.map((thread) => {
        ...
        const currentUpdatedAtMs = new Date(thread.updatedAtIso).getTime()
        if (Number.isFinite(currentUpdatedAtMs) && currentUpdatedAtMs >= updatedAtMs) return thread
        changed = true
        groupChanged = true
        return { ...thread, updatedAtIso }
      })
      return groupChanged ? { ...group, threads: nextThreads } : group
    })

    return changed ? nextGroups : groups
  }

  function applySessionThreadTimestamp(notification: RpcNotification): void {
    if (notification.method !== 'bridge/thread-session-updated') return
    const params = asRecord(notification.params)
    const threadId = readString(params?.threadId).trim()
    const mtimeMs = readNumber(params?.mtimeMs)
    if (!threadId || mtimeMs === null || mtimeMs <= 0) return

    const updatedAtIso = new Date(mtimeMs).toISOString()
    loadedThreadListGroups = updateThreadTimestamp(loadedThreadListGroups, threadId, updatedAtIso)
    const nextSourceGroups = updateThreadTimestamp(sourceGroups.value, threadId, updatedAtIso)
    if (nextSourceGroups === sourceGroups.value) return
    sourceGroups.value = nextSourceGroups
    applyThreadFlags()
  }

  function appendBridgeSubmittedUserMessage(threadId: string, message: UiMessage): void {
    const existing = persistedMessagesByThreadId.value[threadId] ?? []
    if (hasMatchingUserMessage(message, existing)) return
    setPersistedMessagesForThread(threadId, [...existing, message])
  }
  ...
  function applyRealtimeUpdates(notification: RpcNotification): void {
    ...
    const bridgeUserMessage = readBridgeSubmittedUserMessage(notification)
    if (bridgeUserMessage) {
      appendBridgeSubmittedUserMessage(bridgeUserMessage.threadId, bridgeUserMessage.message)
      const bridgeTurnId = bridgeUserMessage.message.turnId?.trim() ?? ''
      if (bridgeTurnId) {
        activeTurnIdByThreadId.value = {
          ...activeTurnIdByThreadId.value,
          [bridgeUserMessage.threadId]: bridgeTurnId,
        }
        acknowledgeDesktopTurn(bridgeUserMessage.threadId, bridgeTurnId)
      }
      setThreadInProgress(bridgeUserMessage.threadId, true)
      setTurnSummaryForThread(bridgeUserMessage.threadId, null)
      if (eventUnreadByThreadId.value[bridgeUserMessage.threadId]) {
        eventUnreadByThreadId.value = omitKey(eventUnreadByThreadId.value, bridgeUserMessage.threadId)
      }
      return
    }
    ...
    applySessionThreadTimestamp(notification)
    ...
    if (startedTurn) {
      ...
      acknowledgeDesktopTurn(startedTurn.threadId, startedTurn.turnId)
    ...
    if (completedTurn) {
      ...
      clearAcknowledgedDesktopTurn(completedTurn.threadId)
    ...
    if (notification.method === 'turn/completed') {
      ...
      if (completedThreadId) {
        ...
        clearAcknowledgedDesktopTurn(completedThreadId)
  ...
  async function loadMessages(threadId: string, options: { silent?: boolean; force?: boolean } = {}) {
    ...
    const loadPromise = (async () => {
      ...
      const preserveDesktopBridgeTurn = shouldPreserveAcknowledgedDesktopTurn(
        threadId,
        inProgress,
        options.force === true,
      )
      const effectiveInProgress = inProgress || preserveDesktopBridgeTurn
      const effectiveActiveTurnId = activeTurnId || (preserveDesktopBridgeTurn
        ? acknowledgedDesktopTurnByThreadId.value[threadId]?.turnId ?? ''
        : '')
      ...
      setThreadInProgress(threadId, effectiveInProgress)
      ...
      if (inProgress || activeTurnId) {
        clearAcknowledgedDesktopTurn(threadId)
      } else if (!preserveDesktopBridgeTurn) {
        clearAcknowledgedDesktopTurn(threadId)
      }
      if (!effectiveInProgress) {
        clearCompletedTurnLiveState(threadId)
  ...
  async function startTurnForThread(
    ...
    try {
      ...
      if (startedTurnId) {
        ...
        acknowledgeDesktopTurn(threadId, startedTurnId)
  ...
  async function syncFromNotifications(): Promise<void> {
    ...
    const threadIdsToForceRefresh = new Set(pendingForcedThreadMessageRefresh)
    ...
    try {
      ...
      if (shouldRefreshActiveThread) {
        await loadMessages(activeThreadId, {
          silent: true,
          force: threadIdsToForceRefresh.has(activeThreadId),
        })
      }
    } catch {
