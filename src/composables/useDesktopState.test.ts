...
describe('realtime bridge sync', () => {
  it('shows a selected-thread user message before Desktop turn/start resolves', async () => {
    ...
    const state = useDesktopState()
    state.primeSelectedThread('thread-sync')

    const sendPromise = state.sendMessageToSelectedThread('visible immediately')
    ...
    expect(state.messages.value.map((message) => `${message.role}:${message.text}`)).toContain(
  ...
  it('renders submitted user messages from bridge notifications without duplicates', async () => {
    installTestWindow()
    let notificationHandler: (notification: { method: string; params?: unknown }) => void = () => {}
    gatewayMocks.subscribeCodexNotifications.mockImplementation((handler) => {
      notificationHandler = handler
    ...
    expect(state.messages.value.map((message) => `${message.role}:${message.text}:${message.messageType}`)).toEqual([
      'user:sent from phone:userMessage.optimistic',
    ])
    expect(state.selectedLiveOverlay.value?.activityLabel).toBe('Thinking')
  })

  it('reloads the active thread when the desktop Codex session file changes', async () => {
    installTestWindow()
    vi.mocked(window.setTimeout).mockImplementation(((callback: TimerHandler) => {
      if (typeof callback === 'function') {
        void Promise.resolve().then(() => callback())
    ...
    gatewayMocks.resumeThread.mockResolvedValue(null)
    gatewayMocks.getThreadGroupsPage.mockResolvedValue({
      groups: [{ projectName: 'project', threads: [thread('thread-sync', '/tmp/project')] }],
      nextCursor: null,
    })
      ...
      .mockResolvedValueOnce({
        messages: [
          ...
          },
          {
            id: 'message-2',
            role: 'user',
            text: 'sent from desktop Codex',
            messageType: 'userMessage',
            createdAtIso: '2026-07-09T00:00:01.000Z',
          },
        ],
        inProgress: true,
    ...
    await state.refreshAll({ includeSelectedThreadMessages: false })
    state.primeSelectedThread('thread-sync')
    await state.loadMessages('thread-sync')
    state.startPolling()
    ...
    expect(state.messages.value.map((message) => message.text)).toContain('sent from desktop Codex')
    expect(state.selectedLiveOverlay.value?.activityLabel).toBe('Thinking')
    expect(state.projectGroups.value[0]?.threads[0]?.updatedAtIso).toBe('2026-07-09T01:20:01.000Z')
  })

  it('keeps a Desktop-acknowledged real turn in progress across an immediate stale thread reload', async () => {
    installTestWindow()
    gatewayMocks.getPendingServerRequests.mockResolvedValue([])
    gatewayMocks.resumeThread.mockResolvedValue({
      model: '',
    ...
    gatewayMocks.startThreadTurn.mockResolvedValue('turn-real-1')
    ...
    const state = useDesktopState()
    state.primeSelectedThread('thread-cdp')

    await state.sendMessageToSelectedThread('run a slow command')
  ...
  it('shows and interrupts a Desktop-originated active turn with its real turn id', async () => {
    installTestWindow()
    let notificationHandler: (notification: { method: string; params?: unknown }) => void = () => {}
    gatewayMocks.subscribeCodexNotifications.mockImplementation((handler) => {
      notificationHandler = handler
    ...
    const state = useDesktopState()
    state.primeSelectedThread('thread-desktop-origin')
    state.startPolling()
    notificationHandler({
      method: 'turn/started',
      params: {
        threadId: 'thread-desktop-origin',
        turn: { id: 'turn-desktop-origin', status: 'inProgress' },
      },
    })
    ...
    expect(gatewayMocks.interruptThreadTurn).toHaveBeenCalledWith(
      'thread-desktop-origin',
      'turn-desktop-origin',
    )
    expect(state.selectedLiveOverlay.value).toBeNull()
  })

  it('keeps the Desktop turn active when interrupt transport fails', async () => {
    installTestWindow()
    let notificationHandler: (notification: { method: string; params?: unknown }) => void = () => {}
    gatewayMocks.subscribeCodexNotifications.mockImplementation((handler) => {
      notificationHandler = handler
    ...
    gatewayMocks.getPendingServerRequests.mockResolvedValue([])
    gatewayMocks.interruptThreadTurn.mockRejectedValue(new Error('Desktop CDP disconnected'))

    const state = useDesktopState()
    state.primeSelectedThread('thread-interrupt-failure')
    state.startPolling()
    notificationHandler({
      method: 'turn/started',
      params: {
        threadId: 'thread-interrupt-failure',
        turn: { id: 'turn-failure', status: 'inProgress' },
      },
    })
    ...
    expect(state.selectedLiveOverlay.value?.activityLabel).toBe('Thinking')
    expect(state.error.value).toContain('Desktop CDP disconnected')
  })
})

describe('provider model selection', () => {
  it('loads Codex models only from the Desktop model catalog', async () => {
    installTestWindow()
