  ...
  return {
    ...
    attachWebSocket: (server: HttpServer) => {
      ...
      server.on('upgrade', (req: IncomingMessage, socket, head) => {
        ...
        if (url.pathname === '/codex-api/agent/ws') {
          agentWss.handleUpgrade(req, socket, head, (ws: WebSocket) => {
      ...
      wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
        const url = new URL(req.url ?? '', 'http://localhost')
        const requestedDeviceId = url.searchParams.get('deviceId')?.trim() || undefined
      ...
      agentWss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
        bridge.acceptDesktopAgentSocket(ws, {
