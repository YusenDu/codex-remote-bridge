...
} from '../server/codexDesktopCdp/desktopRpcRouter.js'
...
async function startServer(options: {
  ...
  const useLocalCodex = shouldUseLocalCodexAppServer(desktopBridgeMode)
  const codexCommand = useLocalCodex
    ? ensureCodexInstalled() ?? resolveCodexCommand()
    : null
  if (codexCommand) {
  ...
  if (useLocalCodex && options.login && !hasCodexAuth()) {
