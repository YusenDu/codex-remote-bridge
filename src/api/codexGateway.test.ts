...
describe('startThreadTurn collaboration mode payloads', () => {
  ...
  it('attaches realtime bridge metadata to turn/start requests', async () => {
    const { requests } = mockRpcFetch()

    await startThreadTurn(
      'thread-1',
      'hello',
      ['/codex-local-image?path=a.png'],
      'gpt-5.4',
      'medium',
      [{ name: 'browser', path: '/skills/browser/SKILL.md' }],
      [{ label: 'README.md', path: 'README.md', fsPath: '/tmp/README.md' }],
      'default',
    )

    const metadata = requests[0].params.__codexWebBridge as Record<string, unknown>
    ...
    expect(typeof metadata.clientId).toBe('string')
    expect(typeof metadata.submissionId).toBe('string')
